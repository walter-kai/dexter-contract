...
contract TestDCABot is DexterHook {
    constructor(IPoolManager _poolManager, address feeRecipient, address executor) DexterHook(_poolManager, feeRecipient, executor) {}
...
contract SimpleStratTest is Test {
    ...
    uint256 dcaId = bot.createDCAStrategy{value: valueToSend}(poolParams, dca, 100, block.timestamp + 1 hours);
        ...
        // Get order info to check gas funded - access the struct directly
        DexterHook.OrderInfo memory orderInfo;
        (
            orderInfo.user,
            orderInfo.totalAmount,
            orderInfo.poolKey,
            orderInfo.expirationTime,
            orderInfo.maxSlippageBps,
            orderInfo.takeProfitPercent,
            orderInfo.ticksLength,
            orderInfo.maxSwapOrders,
            orderInfo.zeroForOne,
            orderInfo.status,
            orderInfo.isPerpetual,
            orderInfo.priceDeviationPercent,
            orderInfo.priceDeviationMultiplier,
            orderInfo.baseSwapAmount,
            orderInfo.swapOrderMultiplier,
            orderInfo.gasFunded,
            orderInfo.gasUsed
        ) = bot.orders(dcaId);

        assertEq(orderInfo.gasFunded, expected, "gas allocated should equal expected");

        console.log("expected gas", expected);
        console.log("allocated gas", orderInfo.gasFunded);
    }
}
