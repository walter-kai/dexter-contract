...
contract DexterHookComprehensiveTest is Test, Deployers {
    ...
    function setUp() public {
        ...
        (address hookAddress, bytes32 salt) = HookMiner.find(
            address(this),
            flags,
            type(DexterHook).creationCode,
            abi.encode(manager, FEE_RECIPIENT, EXECUTOR)
        );
  ...
  function test_beforeSwap() public {
    ...
    function test_gasRefund() public {
        ...
        // User should have received some gas refund (if compensation pool has funds)
        // Note: This might not always happen depending on gas usage and compensation pool balance
    }
    ...
    function test_createDCAStrategy_MaxSwapOrders() public {
        ...
        uint256 dcaId = dcaBot.createDCAStrategy{value: 1 ether}(
            poolParams,
            dcaParams,
            100,
            block.timestamp + 1 hours
    ...
    function test_createDCAStrategy_InsufficientGas() public {
        ...
        vm.expectRevert();
        dcaBot.createDCAStrategy{value: 0}(
            poolParams,
            dcaParams,
            100,
            block.timestamp + 1 hours
    ...
    function test_stressTest_LargeAmounts() public {
        ...
        uint256 dcaId = dcaBot.createDCAStrategy{value: 10 ether}(
            poolParams,
            dcaParams,
            100,
            block.timestamp + 1 hours
    ...
    function _createTestDCAStrategy(address user) internal returns (uint256 dcaId) {
        ...
        dcaId = dcaBot.createDCAStrategy{value: gasAllocation}(
            poolParams,
            dcaParams,
            100,
            block.timestamp + 1 hours
